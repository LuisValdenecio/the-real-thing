...
for (var x = 0; Number(table.attributes[1]) > x; x++) {
    table.children[1].appendChild();
}

function createElement(tag, className, attribue) {

}

function reverseString(string) {
    if (string.length == 1) return string.charAt(string.length-1);
    return string.charAt(string.length-1) + reverseString(string.slice(string.length - 1));  // passo de recursão
}
