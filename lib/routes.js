...
module.exports = {
			...
			app : function (app,io, users, dadosFaltas, professores, subjecInfo, studentsFaults, schoolRegister, classesInfo, turmaInfo, teacherDetails) {
					...
					*/
					app.get(/\/turma_(\w+-)(\w+-)(\w+-)(\w+-)(\w+)(_novoEstudante)$/, function(req, res) {
						...
						if (req.isAuthenticated()) {
							res.render('estudantes' , {from: 'NovoEstudents'});
						} else {
							res.redirect('login');
						}
					...
					*/
					app.post(/\/turma_(\w+-)(\w+-)(\w+-)(\w+-)(\w+)(_novoEstudante)$/, function(req, res) {
					...
					*/
					app.get(/\/turma_(\w+-)(\w+-)(\w+-)(\w+-)(\w+)(_ficheiroExcel)$/, function(req, res) {
					...
					*/
					app.get(/\/turma_\w+/, function(req, res){
						...
						if (req.isAuthenticated()) {
								...
								client.query('SELECT "nome", "disciplina_nome", "foto" FROM professor WHERE turma_id = $1',
									[turmaCod], function(err,result){
