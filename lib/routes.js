					...
					try {
						...
						client.query('SELECT "nome", "estudante_cod", "sobrenome", "foto" FROM estudante WHERE nome_turma = $1', [className], function(err, result) {
								if(err) console.log(err);
									...
									// another query for student faults.
									for (var x = 0; x < users.length; x++) {
											client.query('SELECT "nome_disciplina", "numero_de_faltas" FROM falta_ausencia WHERE estudante_cod = $1 AND NOT numero_de_faltas < 1', [users[x]["estudante_cod"]], function(err, result){
												if (err) console.log(err);
												else {
					...
					try {
							...
							client.query('SELECT "estudante_cod", "nome", "sobrenome" FROM "estudante" WHERE nome_turma = $1', [className], function(err, result) {
								if (err) {
									...
									// dangareoug approach
									for (var x of dataStudents) {

										client.query('INSERT INTO falta_ausencia (estudante_cod , nome , sobrenome , nome_disciplina , trimestre , numero_de_faltas) VALUES ($1, $2, $3, $4, $5, $6)',[x["estudante_cod"], x["nome"], x["sobrenome"], subject, "3º", Number(req.body[`${x["estudante_cod"]}`])],function(err, result){
											if (err) {
												console.log(err);
											} else {
												 client.query('COMMIT');
