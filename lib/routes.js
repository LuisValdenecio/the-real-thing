...
module.exports = {
			...
			app : function (app,io, users, dadosFaltas, professores, subjecInfo) {
					...
					try {
							...
							client.query('SELECT "estudante_cod", "nome", "sobrenome" FROM "estudante" WHERE nome_turma = $1', [className], function(err, result) {
								if (err) {
									...
									dataStudents = result.rows;


									for (var x = 0; x < dataStudents.length; x++) {


										client.query('INSERT INTO falta (estudante_cod , nome , sobrenome , nome_disciplina , trimestre , falta_indisciplinada, falta_ausencia, falta_material) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',[dataStudents[x]["estudante_cod"], dataStudents[x]["nome"], dataStudents[x]["sobrenome"], subject, "3º",
										Number(Number(req.body[`${dataStudents[x]["estudante_cod"]}`][0])), Number(Number(req.body[`${dataStudents[x]["estudante_cod"]}`][1])), Number(req.body[`${dataStudents[x]["estudante_cod"]}`][2]) ],function(err, result){
											if (err) {
												console.log(err);
											} else {
												 client.query('COMMIT');
