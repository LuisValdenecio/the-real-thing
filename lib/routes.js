...
module.exports = {
			...
			app : function (app,io, users, dadosFaltas, professores, subjecInfo, studentsFaults, schoolRegister, classesInfo, turmaInfo) {
					...
					*/
					app.get(/^\/\w+\D+?\w+?\_$/, function(req, res) {
						if (req.isAuthenticated()) {
					...
					*/
					app.get(/\/turma_\w+/, function(req, res){
					...
					app.post(/\/turma_\w+/, upload.single('imageupload') , function(req, res){
						...
						try {
							...
							*/
							...
							client.query('INSERT INTO coordenador (coordenador_cod, nome, email, nome_usuario, palavrapasse, escola_cod, foto) VALUES ($1, $2, $3, $4, $5, $6, $7)',
								[uniqueKey, req.body.name, req.body.email, req.body.username , req.body.password, classesInfo[0]["escola_cod"], req.file.filename], function(err,result){
									...
									if (err) {
										...
										client.query('INSERT INTO professor_disciplina (curso_nome, coordenador_cod, nome_class, turma_id, disciplina_nome ) VALUES ($1, $2, $3, $4, $5)',
											[req.body.courseName, uniqueKey, req.body.className, req.body.classCod, req.body.subjectName], function(err,resu) {
