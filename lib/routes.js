							...
							client.query('SELECT "estudante_cod", "nome", "sobrenome" FROM "estudante" WHERE nome_turma = $1', [className], function(err, result) {
								if (err) {
									...
									for (var x of dataStudents) {
										client.query('INSERT INTO falta (estudante_cod , nome , sobrenome , nome_disciplina , trimestre , falta_indisciplinada, falta_ausencia, falta_material) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',[x["estudante_cod"], x["nome"], x["sobrenome"], subject, "3º",
										Number(req.body[`${x["estudante_cod"]}`][0]), Number(req.body[`${x["estudante_cod"]}`][0]), Number(req.body[`${x["estudante_cod"]}`][0]) ],function(err, result){
											if (err) {
												console.log(err);
											} else {
												 client.query('COMMIT');
