					...
					app.get(/\/turma_(\w+-)(\w+-)(\w+-)(\w+-)(\w+)(_estudantes)$/, function(req, res) {
						...
						if (req.isAuthenticated()) {
							...
							try {
								...
								client.query('SELECT "nome", "foto", "estudantecod", "genero","idade", "turma_id" FROM estudante', function(err,result) {
					...
					app.get(/\/turma_(\w+-)(\w+-)(\w+-)(\w+-)(\w+)(_livrodeponto)$/, function(req, res) {
						...
						if (req.isAuthenticated()) {
							...
							try {
								...
								// consulte todas as as faltas dos _estudantes
					...
					*////////////////////////////////////////////////////////////////////////////////////
					app.post(/\/turma_(\w+-)(\w+-)(\w+-)(\w+-)(\w+)(_livrodeponto)$/, function(req,res){
						...
						try {
							...
							// itera sobre cada estudante e comfirma se ja tem alguma falta nesta disciplina
							for (let x in req.body) {

								// no caso de estar no primeiro atributo, enão continua no próxmio ponto
								if (x == "disciplina") {
									continue;
								} else {

									// em primeiro lugar, selecione o que estiver na tabela coluna
									client.query('SELECT "ausencia", "material", "disciplinar", "participacao" FROM falta WHERE estudantecod = $1 AND disciplina_nome = $2',
									 [x, req.body["disciplina"]], function(err, resp) {

										 if (err) {
											 console.log(err);
										 } else {

											 // caso não existam ainda estudantes com faltas marcadas nesta disciplina então:
											 // marque-as
											 if (resp.rows.length == 0) {
												 ...
												 client.query('INSERT INTO falta (falta_cod, estudantecod, turma_id, disciplina_nome, data, ausencia, material, disciplinar, participacao) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)',
												 		[uuidv4(), x, thisClass, req.body["disciplina"], helpers.getDate(), req.body[x][0], req.body[x][1], req.body[x][2], req.body[x][3] ],
											 ...
											 // caso esta disciplina ja tenha marcada alguma falta, então, actualize o número
											 // de faltas, somando o nº de faltas anterior a 0 ou 1 dependo se o estudante faltou ou não
											 else {

												 client.query('UPDATE falta SET "ausencia"=$1, "material"=$2, "disciplinar"=$3, "participacao"=$4  WHERE estudantecod=$5',
													 	[
															Number(resp.rows[0]["ausencia"]) + Number(req.body[x][0]),
															Number(resp.rows[0]["material"]) + Number(req.body[x][1]),
															Number(resp.rows[0]["disciplinar"]) + Number(req.body[x][2]),
															Number(resp.rows[0]["participacao"]) + Number(req.body[x][3]),
														 	x
													 	],
					...
					app.get(/\/turma_(\w+-)(\w+-)(\w+-)(\w+-)(\w+)(_horario)$/, function(req, res) {
						...
						if (req.isAuthenticated()) {
							res.render('horario', {from: 'horario', usuarioNome : req.session.passport.user[0].userName});
						} else {
							res.redirect('login');
						}
					...
					app.get(/\/turma_(\w+-)(\w+-)(\w+-)(\w+-)(\w+)(_novoEstudante)$/, function(req, res) {
						...
						if (req.isAuthenticated()) {
							...
							try {
								...
								client.query('SELECT "nome","foto","genero","idade", "turma_id", "estudantecod" FROM estudante', function(err,result) {
					...
					app.post(/\/turma_(\w+-)(\w+-)(\w+-)(\w+-)(\w+)(_novoEstudante)$/, upload.single('imageupload'), function(req, res) {
						...
						try {
							...
							client.query('INSERT INTO estudante (estudantecod, nome, foto, turma_id, email, genero, idade) VALUES ($1, $2, $3, $4, $5, $6, $7)',
