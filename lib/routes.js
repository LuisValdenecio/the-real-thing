...
module.exports = {
			...
			app.get('/account', function (req, res, next) {
				if(req.isAuthenticated()){
					...
					try {
						...
						client.query('SELECT "nome", "sobrenome", "foto" FROM estudante WHERE nome_turma = $1', [className], function(err, result) {
			...
			app.get('/Proj.Tecn', function(req, res) {
				if (req.isAuthenticated()) {
					var subject = req.route.path.replace("/","");
					var className = req.session.passport.user[0].className;
			...
			app.get('/Cont.Analitica', function(req, res) {
				if (req.isAuthenticated()) {
					var subject = req.route.path.replace("/","");
					var className = req.session.passport.user[0].className;
					res.render('faltas', {path : subject});
			...
			app.get('/Cont.Financeira', function(req, res) {
				if (req.isAuthenticated()) {
					var subject = req.route.path.replace("/","");
					var className = req.session.passport.user[0].className;
					res.render('faltas', {path : subject});
			...
			app.get('/A.E.F.', function(req, res) {
				if (req.isAuthenticated()) {
					var subject = req.route.path.replace("/","");
					var className = req.session.passport.user[0].className;
					res.render('faltas', {path : subject});
			...
			app.get('/Empreend', function(req, res) {
				if (req.isAuthenticated()) {
					var subject = req.route.path.replace("/","");
					var className = req.session.passport.user[0].className;
					res.render('faltas', {path : subject});
