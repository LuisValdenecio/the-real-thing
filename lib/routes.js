			...
			*/
					...
					bcrypt.hash(req.body.password, 10, function(err, hash) {
						...
						JSON.stringify(client.query('SELECT nome FROM "escola" WHERE "nome_usuario"=$1', [req.body.username], function(err, result) {
							...
							// this userName hasnt been taken already
									...
									client.query('INSERT INTO escola (nome, email, nome_usuario, provincia, senha) VALUES ($1, $2, $3, $4, $5)',
									[req.body.firstName, req.body.email, req.body.username , req.body.provincia, hash], function(err, result) {
										if(err) {
											console.log(err);
										} else {
											client.query('COMMIT');
						...
						bcrypt.hash(req.body.password, 10, function(err, hash) {
							...
							JSON.stringify(client.query('SELECT coordenador_cod FROM "coordenador" WHERE "nome_usuario"=$1', [req.body.username], function(err, result) {
								if(result.rows[0]) {
								    ...
								    client.query('INSERT INTO coordenador (coordenador_cod , nome, email, escola, nome_usuario, palavrapasse) VALUES ($1, $2, $3, $4, $5, $6)',
										 [uuidv4(), req.body.firstName, req.body.email, req.body.escola, req.body.username, hash], function(err, result) {
											if(err) {
												console.log(err);
											} else {
										    client.query('COMMIT');
			...
			app.get('/account', function (req, res, next) {
				if(req.isAuthenticated()){
					...
					try {
						...
						client.query('SELECT "nome", "estudante_cod", "sobrenome", "foto" FROM estudante WHERE nome_turma = $1', [className], function(err, result) {
			...
			app.post('/Estudante', upload.single('imageupload'), function(req, res) {
					try {
						...
						client.query('INSERT INTO estudante (estudante_cod , nome, sobrenome, nome_turma, foto) VALUES ($1, $2, $3, $4, $5)', [uuidv4(), req.body.firstName, req.body.lastName, req.body.classname, req.file.filename], function(err, result) {
								if(err) {
									console.log(err);
								} else {
									client.query('COMMIT');
			...
			// registo de cursos ou professores
			app.post('/tecnico', function(req, res) {
				if (req.isAuthenticated()) {
					...
					try {
						...
						// 1st insertion of data.
						for (let classe in req.body) {
							client.query('INSERT INTO turma (nome_turma) VALUES ($1) ',[classe],function(err,result){
								if (err) console.log(err);
								else {
									console.log("sucess");
...
passport.use('localOne', new  LocalStrategy({passReqToCallback : true}, (req, username,password, done) => {
	...
	function loginAttempt() {
		...
		try{
			client.query('BEGIN')
			var currentAccountsData = JSON.stringify(client.query('SELECT coordenador_cod, "nome", "nome_usuario", "email", "palavrapasse", "nome_turma" FROM "coordenador" WHERE "nome_usuario"=$1', [username], function(err, result) {
...
passport.use('localTwo', new LocalStrategy({passReqToCallback : true}, (req, a,b, done)=>{
	return done(null, [{username : "luis"}]);
}));
